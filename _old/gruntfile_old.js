module.exports = function(grunt)
{
 	...
 	// TASK SETUP
  	...
  	grunt.initConfig({
		...
		exec:
		{
					...
					switch(response_code)
					{
						case '200':
							...
							parseString(arr[1], function (err, result) {
								...
								grunt.file.write('links.json', JSON.stringify(links2, null, 2)); // Save the links & link ids
							});					
							break;
						default:
			...
			count_clicks:
			{
				cmd: 'curl -i -H Accept: application/xml -u ' + global_config.streamsend_api_credentials + ' "https://app.streamsend.com/blasts/22097791/clicks/count.xml"',
				callback: function (error, stdout, stderr)
				{
					var response_code = get_http_status(stdout);
					switch(response_code)
					{
						case '200':
							...
							arr = stdout.split(/\r?\n\r?\n/, 2);
							parseString(arr[1], function (err, result) {
								session.clicks = result.count._;
								console.log('Click count is ' + result.count._);
							});
							break;
						default:
							console.log('ERROR - Clicks could not be counted - HTTP Response Code: ' + response_code);
							break;
					}
				}
			},
			...
			count_views:
			{
				cmd: 'curl -i -H Accept: application/xml -u ' + global_config.streamsend_api_credentials + ' "https://app.streamsend.com/blasts/22097791/views/count.xml"',
				callback: function (error, stdout, stderr)
				{
					var response_code = get_http_status(stdout);
					switch(response_code)
					{
						case '200':
							...
							arr = stdout.split(/\r?\n\r?\n/, 2);
							parseString(arr[1], function (err, result) {
								session.views = result.count._;
								console.log('View count is ' + result.views._);
							});
							break;
						default:
							console.log('ERROR - Views could not be counted - HTTP Response Code: ' + response_code);
							break;
					}
				}
			},
	...
	grunt.registerTask('get-clicks', ['exec:count_clicks', 'get-clicks-looper']);
	grunt.registerTask('get-clicks-looper', 'Look through the pages of clicks, downloading each in turn', function() {
  		// This task is called by get-clicks to loop through the paginated clicks pages, downloading each in turn
  		var pages = Math.ceil(session.clicks / 100);
		console.log('There are ' + pages + ' pages of clicks *max 100 per page)');
		var links = grunt.file.readJSON('links.json');
		var links2 = {};
		...
		{
			grunt.task.run('exec:get_clicks:' + i);
		}
		grunt.task.run('get-clicks-report');
	});
	grunt.registerTask('get-clicks-report', 'Report of the result of compiling all the clicks', function() {
		...
		var unclks2 = [];
		for (link in clicks) unclks[link] = _.sortBy(_.uniq(_.pluck(clicks[link], 'email')), function(email) { return email; }); // Remove duplicates, and put in alphabetical order
		...
		grunt.file.write('unclks.json', JSON.stringify(unclks2, null, 2)); // Unique clicks by link
		grunt.file.write('clicks.json', JSON.stringify(clicks2, null, 2)); // All clicks, including duplicates
		grunt.file.write('people.json', JSON.stringify(people2, null, 2)); // Links clicked by email
